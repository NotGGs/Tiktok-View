...
banner = f"""
 ...
 ╚██████╔╝██║  ██║███████╗╚██████╔╝╚██████╔╝██║  ██║██║╚██████╔╝███████║
  ╚═════╝ ╚═╝  ╚═╝╚══════╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═╝╚═╝ ╚═════╝ ╚══════╝
{trang}                                                         </> Script By Gregorius </>
\033[1;37m────────────────────────────────────────────────────────────
{do}[{trang}{kt_code}{do}] {vang}BOT VIEW TIK TOK MAX SPEED 
{do}[{trang}{kt_code}{do}] \033[1;35mADMIN:\033[1;36m Gregorius
{do}[{trang}{kt_code}{do}] {dac_biet}Script :\033[1;37m Free Premium
\033[1;37m────────────────────────────────────────────────────────────"""
        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(f'{do}[{vang} Gregorius View {do}] {do}[{xanh_la}View Added{do}] {do}{xanh_duong}+: {trang}{success}{do} View')
                    success += 1
                    _lock.release()
            except:
...
def rpsm_loop():
    ...
    while True:
        initial = reqs
        time.sleep(3.5)
        rps = round((reqs - initial) / 2.5, 1)
        rpm = round(rps * 60, 1)


def fetch_proxies():
    url_list =[
        "/sdcard/download/BotViewTiktok/url.txt"
...
if __name__ == "__main__":
    ...
    print(banner)
    try:
        link = input(f'{do}[{trang}{kt_code}{do}] {xanh_la}Input Code Video: {trang}')
        print(f'{trang}- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -')
        __aweme_id = str(
        ...
        exit(f"{do}[{trang}{kt_code}{do}] {do}Invalid Code")
